...
支持 **付费墙（免费试用 5 次 + ¥19.9 永久解锁）**，激活码由后端签发、前端无法伪造。
...
- 付费墙：免费试用 5 次，¥19.9 永久解锁（后端签发激活码）

## 目录结构
```
index.html              前端 H5（含付费墙 UI）
...
5. 游客在微信里打开小程序 → web-view 加载 H5 → 点购买 → 跳转原生支付页 `pages/pay` → `wx.requestPayment` → 成功后回传激活码自动激活
